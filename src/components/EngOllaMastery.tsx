...
export default function EngOllaMastery({onExit,intelligenceContext}:{onExit:()=>void;intelligenceContext:DashboardIntelligenceContext|null}) {
  ...
  return <div id="olaTrickOverlay" role="dialog" aria-modal="true" aria-label="Eng. Olla Executive Mastery">
    ...
    {screen==="modules"&&<section className={`ollaExperience ollaEnter ${direction}`}><header className="ollaMasteryHead"><span>ENG. OLLA</span><h1>EXECUTIVE MASTERY</h1><p>Project Finance <i>•</i> Executive Decision Intelligence</p></header><div className="ollaModuleChoices"><button type="button" className="ollaModuleChoice live" onClick={()=>change("intelligence",moduleIndex,page,"next")}><span className="ollaModuleNumber">LIVE</span><span className="ollaModuleText"><b>Live Project Intelligence</b><small>Read the current controlled charts and tables, identify risk and benefit, then decide the action.</small></span><span className="ollaModuleArrow">→</span></button>{OLLA_MODULES.map((item,index)=><button type="button" className="ollaModuleChoice" key={item.id} onClick={()=>chooseModule(index)}><span className="ollaModuleNumber">{item.number}</span><span className="ollaModuleText"><b>{item.title}</b><small>{item.description}</small></span><span className="ollaModuleArrow">→</span></button>)}</div></section>}
    {screen==="intelligence"&&<section className={`ollaExperience ollaLiveExperience ollaEnter ${direction}`}><LiveProjectIntelligence context={intelligenceContext} onBack={goBack}/></section>}
