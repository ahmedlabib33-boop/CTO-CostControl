...
export function GroupedBarChart({labels,series,onSelect,horizontal=false,valueFormatter=money}:{labels:string[];series:{label:string;values:(number|null|undefined)[]}[];onSelect?:(i:number)=>void;horizontal?:boolean;valueFormatter?:(v:number)=>string}) {
  const hasData=labels.length>0&&series.some(s=>s.values.some(v=>typeof v==="number"&&Number.isFinite(v)));
  if(!hasData)return <div className="chartEmpty">Source data unavailable for this reporting period.</div>;
  const all=series.flatMap(s=>s.values.map(v=>Math.abs(safe(v)))); const max=Math.max(1,...all);
  if(horizontal) return <div className="hbarChart">{labels.map((label,i)=><button type="button" className="hbarRow" key={`${label}-${i}`} onClick={()=>onSelect?.(i)}><span className="hbarLabel" title={label}>{label}</span><span className="hbarTracks">{series.map((s,si)=>{const v=safe(s.values[i]);return <span className="hbarTrack" key={`${s.label}-${si}`}><i style={{width:`${Math.abs(v)/max*100}%`,background:COLORS[si%COLORS.length]}}/><b>{valueFormatter(v)}</b><em>{s.label}</em></span>})}</span></button>)}</div>;
...
export function BubbleChart({points,onSelect}:{points:{label:string;group?:string;x:number;y:number;size:number;detail?:string}[];onSelect?:(i:number)=>void}) {
  if(!points.length)return <div className="chartEmpty">Source data unavailable for this reporting period.</div>;
...
function Legend({series}:{series:string[]}) { return <div className="legendline">{series.map((s,i)=><span key={`${s}-${i}`}><i className="legend" style={{background:COLORS[i%COLORS.length]}}/>{s}</span>)}</div> }
