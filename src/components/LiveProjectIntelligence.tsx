...
export default function LiveProjectIntelligence({ context, onBack }: { context: DashboardIntelligenceContext | null; onBack: () => void }) {
  const [scope, setScope] = useState<Scope>("current"), [view, setView] = useState<LiveView>("overview"), [family, setFamily] = useState("ALL"), [kind, setKind] = useState<InsightKind | "ALL">("ALL"), [status, setStatus] = useState<InsightStatus | "ALL">("ALL"), [settings, setSettings] = useState(false);
