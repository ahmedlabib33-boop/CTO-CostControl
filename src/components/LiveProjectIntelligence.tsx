...
export default function LiveProjectIntelligence({ context, onBack }: { context: DashboardIntelligenceContext | null; onBack: () => void }) {
  ...
  return <section className="liveIntelligence"><header className="liveIntelligenceHead"><div><span className="ollaEyebrow">Eng. OLLA · Live Project Intelligence</span><h1>Decision Intelligence</h1><p>Every registered application surface is read from its current controlled JSON. No accuracy percentage is invented: compatible evidence is assessed, while missing, conflicting or unsupported evidence remains Unable to assess.</p></div><div className="liveHeadActions"><button type="button" onClick={() => setSettings(true)}>Threshold Settings</button><button type="button" onClick={onBack}>← Mastery Home</button></div></header><div className="liveScope"><button className={scope === "application" ? "active" : ""} onClick={() => setScope("application")}>Entire app</button><button className={scope === "current" ? "active" : ""} onClick={() => setScope("current")}>Current page</button>{context?.kind === "project" && <button className={scope === "project" ? "active" : ""} onClick={() => setScope("project")}>Whole project</button>}<button className={scope === "portfolio" ? "active" : ""} onClick={() => setScope("portfolio")}>Portfolio</button></div>{scope === "application" && <div className={`liveCoverage ${application.error ? "warning" : ""}`}><div><span>Application surface coverage</span><b>{application.loading ? "Loading current registry…" : coverage ? `${coverage.coveredSurfaces} / ${coverage.expectedSurfaces} surfaces registered` : "Registry unavailable"}</b><small>{coverage ? `${coverage.coveragePct.toFixed(0)}% structural coverage · ${results.filter(item => item.status === "unavailable").length} evidence gap(s)` : "No conclusion is produced until controlled data loads."}</small></div><div><span>Live source check</span><b>{application.refreshedAt ? new Date(application.refreshedAt).toLocaleTimeString("en-GB") : "Waiting"}</b><small>{application.error || "Registry fingerprints are rechecked every 15 seconds; data reloads only when a revision changes."}</small></div></div>}<nav className="liveViewTabs" aria-label="Decision intelligence pages">{([{ id: "charts", label: "Charts & Explanations", note: "All app Charts · second-layer reading" }, { id: "overview", label: "Analysis & Tables", note: "All app analysis · interpreted" }, { id: "decisions", label: "Risk & Decisions", note: "All app risks · actions and mitigation" }, { id: "evidence", label: "Evidence & Sources", note: "Rules, sources, mappings and revisions" }] as { id: LiveView; label: string; note: string }[]).map(item => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => setView(item.id)}><b>{item.label}</b><span>{item.note}</span></button>)}</nav><div className="liveViewViewport" key={view}>
