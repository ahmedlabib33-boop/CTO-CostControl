...
const OLA_RISE_OPEN_EVENT="ola-rise:open";
type OlaRiseWindow=Window&{__OLA_RISE_OPEN_REQUESTED__?:boolean};
const requestOlaRiseOpen=()=>{const host=window as OlaRiseWindow;host.__OLA_RISE_OPEN_REQUESTED__=true;host.dispatchEvent(new Event(OLA_RISE_OPEN_EVENT))};
...
function OlaOverlay(){
 ...
 useEffect(()=>{
  ...
  const openRise=()=>{(window as OlaRiseWindow).__OLA_RISE_OPEN_REQUESTED__=false;reveal("rise")};
  window.addEventListener("keydown",key);window.addEventListener("pointerup",touch,{passive:true});window.addEventListener(INTELLIGENCE_CONTEXT_EVENT,intelligence);window.addEventListener(OLA_RISE_OPEN_EVENT,openRise);
  if((window as OlaRiseWindow).__OLA_RISE_OPEN_REQUESTED__)openRise();
  return()=>{window.removeEventListener("keydown",key);window.removeEventListener("pointerup",touch);window.removeEventListener(INTELLIGENCE_CONTEXT_EVENT,intelligence);window.removeEventListener(OLA_RISE_OPEN_EVENT,openRise);document.documentElement.classList.remove("trick-open")}
 },[]);
