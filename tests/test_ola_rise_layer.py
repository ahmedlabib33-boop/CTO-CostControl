...
class OlaRiseLayerTests(unittest.TestCase):
    ...
    def test_visual_walkthrough_and_main_game_button_are_directly_available(self):
        ...
        self.assertIn("ola.visible = false", script)
        self.assertIn("function cinematicDialogueSprite", script)
        self.assertIn("visibleCinematicConversations", script)
        self.assertIn("ambientActorCount", script)
        self.assertIn("body.walkthrough-active #game > :not(#world3d):not(#walkthroughPanel)", css)
    ...
    def test_mobile_controls_survive_cancelled_or_unsupported_pointer_capture(self):
        ...
        self.assertIn('release=20260907-v31', html)
        self.assertIn('release=20260907-v31', launcher)
    ...
    def test_day_cycle_sleep_lock_coffee_and_wellbeing_are_visible(self):
        ...
        self.assertIn("فاطمة", script)
        self.assertIn("حصة", script)
        self.assertIn("لولوة", script)
        self.assertIn("زينب", script)
        self.assertEqual(script.count("ambientActor("), 11)
        self.assertEqual(script.count("walks: true"), 4)
        self.assertEqual(script.count("walks: false"), 6)
